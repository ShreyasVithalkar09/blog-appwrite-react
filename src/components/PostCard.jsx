...
function PostCard({ $id, title, featuredImage, $createdAt }) {
  return (
    <Link to={`/post/${$id}`}>
      <div className="card-box w-80 max-h-80 h-56 rounded-md border border-gray-400 p-3 bg-[#FEFCF3] box-border flex flex-col items-center">
        <div className="image w-72 h-2/3 border rounded-md mb-2 box-border">
          <img
            className="w-full h-full object-cover rounded-md box-border bg-white"
